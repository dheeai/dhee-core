...
const Params = Type.Object({
  ...
  keepPrompt: Type.Optional(
    Type.Boolean({
      description:
        "When true, skip the LLM prompt-write phase on the next run and use the existing on-disk prompt file as-is. Use this AFTER you (or the user) hand-edited a `setting_image` / `character_image` / `object_image` prompt JSON file at `prompts/images/{settings,characters,objects}/<name>.json` and only want the IMAGE to regenerate (not the text). These three node types are MERGED — they run LLM-writes-prompt then Comfy-renders-image inside a single node, so a plain invalidate re-runs the LLM and overwrites your edits. `keepPrompt: true` flips a per-node sentinel that overrides the staleness check. The sentinel is automatically cleared after a successful image render. No effect on other node types (silently ignored — their executor path does not read this flag).",
    }),
  ),
});
...
export const dheeInvalidate = defineTool({
  ...
  async execute(_id, params: Static<typeof Params>): Promise<AgentToolResult<InvalidateDetails>> {
    ...
    const result = applyInvalidation(
      ...
      ids,
      params.keepPrompt !== undefined ? { keepPrompt: params.keepPrompt } : {},
    );
