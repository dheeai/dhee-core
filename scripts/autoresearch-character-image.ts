#!/usr/bin/env tsx
/**
 * Autoresearch: optimize character_image_guide.md
 *
 * 1. Generate character image prompts from character profiles + world style
 * 2. Evaluate with binary rubric (14 questions) via claude -p
 * 3. Propose guide improvements via claude -p
 * 4. Save new guide, repeat
 *
 * Usage:
 *   pnpm tsx scripts/autoresearch-character-image.ts [iterations]
 *
...
const GUIDE_PATH = 'prompts/skills/defaults/character_image_guide.md';
const RUBRIC_PATH = 'tests/autoresearch/rubrics/character-image-binary.json';
const OUTPUT_DIR = 'test-output/autoresearch-character-image';

if (!existsSync(OUTPUT_DIR)) mkdirSync(OUTPUT_DIR, { recursive: true });

const rubric = JSON.parse(readFileSync(RUBRIC_PATH, 'utf-8'));

// Test cases: projects with character profiles + world style bibles
interface TestCase {
  projectName: string;
  characterFile: string;  // relative to project dir
  worldStyleFile?: string;
}

function findTestCases(): Array<TestCase & { characterContent: string; worldStyle: string }> {
  const projects = [
  ...
  for (const proj of projects) {
    ...
    const charFile = join(projDir, 'characters', '.md');
...
if (testCases.length === 0) {
  console.error('No test cases found — need projects with characters/.md files');
  process.exit(1);
}
...
function claudeP(prompt: string, jsonSchema?: Record<string, unknown>): string {
  const tmpFile = `/tmp/ar-ci-${Date.now()}.txt`;
...
async function generateCharacterImagePrompt(
  guide: string,
  characterContent: string,
  worldStyle: string,
): Promise<string> {
  const systemPrompt = `You are writing an image generation prompt for a character reference image.
...
${guide}
  ...
  const userContent = worldStyle
    ? `Write an image prompt for this character using the world style below.\n\n## Character Profile\n${characterContent}\n\n## World Style Bible\n${worldStyle}`
    : `Write an image prompt for this character.\n\n## Character Profile\n${characterContent}`;
...
function evaluate(
  ...
  const prompt = `Be strict. Evaluate this character image prompt.

## Character Profile (source material)
${characterContent.substring(0, 3000)}
...
function proposeImprovement(
  ...
  const prompt = `You are optimizing a character image prompt guide for AI video production.

The guide teaches an LLM to write image prompts for character reference images. These images are used as visual references when generating shot images for video production. The prompts go to a text-to-image model (Stable Diffusion / FLUX).
...
async function main() {
  ...
  console.log(`\n=== Starting autoresearch for character image guide ===`);
  ...
  for (let iter = 1; iter <= MAX_ITERATIONS; iter++) {
    ...
    for (const tc of testCases) {
      ...
      const imagePrompt = await generateCharacterImagePrompt(guide, tc.characterContent, tc.worldStyle);
      writeFileSync(join(OUTPUT_DIR, `iter-${iter}-${tc.projectName}.txt`), imagePrompt);

      console.log(`  Evaluating ${tc.projectName}...`);
      const result = evaluate(imagePrompt, tc.characterContent, tc.worldStyle, tc.projectName);
