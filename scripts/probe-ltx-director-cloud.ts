 ...
 *
 *   - Workflow: workflows/cloud/ltx23_director_cloud.json
...
// ── Force Comfy Cloud endpoint ───────────────────────────────────────
// The cloud graph (ltx23_director_cloud.json) uses cloud-available model
// files (gemma_3_12B_it_fp8_scaled instead of the local heretic fp8
// build; the OmniNFT-RL lora + LTX VAEs are present on both). Override
// BEFORE the client is constructed so ComfyUIClient resolves cloud mode.
process.env['COMFY_MODE'] = 'cloud';
process.env['COMFYUI_BASE_URL'] =
  process.env['COMFY_CLOUD_BASE_URL'] ?? 'https://cloud.comfy.org/api';
// COMFY_CLOUD_API_KEY must be supplied externally → sent as X-API-Key.
...
const workflowPath = resolve(
  process.cwd(),
  'workflows/cloud/ltx23_director_cloud.json',
);
