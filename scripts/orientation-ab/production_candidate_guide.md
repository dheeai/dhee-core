...
# LEAD WITH THE FOCAL SUBJECT — HARD CONSTRAINT for all shots

**The first noun phrase of your prompt body is what Klein anchors on.** If a character is inside a vehicle (or behind glass, or framed by an object) and you open with the vehicle/object as the first noun, Klein treats the character as decoration and may DROP them entirely — even when they have a reference slot.
...
**WRONG output (Klein will drop Ruby):**
...
Wrong because: the first noun is "green Lamborghini" — Klein renders the car as subject and the empty driver's seat is just... empty. Ruby's slot 2 gets ignored.
...
Correct because: the first noun is "Ruby" (after the framing preposition). Klein binds the slot 2 to a visible person in the seat.

**General rule:** ask "what is the focal subject Klein must render sharply?" That subject's NAME (or refId-derived noun) must appear in the first sentence — ideally the first noun phrase. Background elements come second.

---

# BANNED WEASEL WORDS for slotted characters — HARD CONSTRAINT

If a character has a reference slot in the `references[]` array, NEVER describe them with the following phrases — Klein interprets these as "deprioritize this slot" and drops the character entirely:
...
**Especially watch for "dark silhouette" as a stylistic adjective phrase.** Even when describing rim-lit characters in atmospheric scenes, NEVER write "Angel's hoodie a dark silhouette against the bright pavement" or "Ruby's outline a sharp silhouette in the doorway" — the word "silhouette" alone tells Klein to render a featureless dark shape, dropping the character's identity entirely. Use concrete features instead.
...
# INLINE VISUAL HOOK on every character's first mention — HARD CONSTRAINT

Each character named in the prose body MUST have a parenthetical 4-8 word visual descriptor on their FIRST mention. Subsequent mentions can drop it.

Format: `[Name] ([descriptor])`. The descriptor must include enough features to disambiguate the character from others in the scene — at minimum, ethnicity/skin-tone, hair, and one distinctive clothing item.

Examples (project: Ruby V3):
- ✓ `Ruby (red-haired, leather jacket)`
- ✓ `Angel (Black man, dark hoodie, short coiled hair)`
- ✓ `the owner (balding white man, button-up shirt)`
- ✗ `Ruby steps forward and Angel mirrors her stance.` ← unhooked, will bleed identity

This rule is **the single most effective fix for the Ruby-as-Angel identity bleed bug**. When Klein has to bind multiple character slots, the inline descriptors give it the token-level anchors it needs to keep them separate.

---

# WORD COUNT — TIGHTEN for close-ups

Old guidance was 80-220 words for fresh frames. Empirically, prompts above ~130 words for close-ups dilute identity binding because Klein has too many competing signals to weigh.

Revised targets:
- **Close-up / extreme close-up** (character face fills frame): **60-130 words**
- **Medium / medium close-up** (character chest-up or waist-up): **100-180 words**
- **Wide / extreme wide / establishing**: **120-220 words**

For close-ups specifically, RESIST adding rasa-style atmospheric paragraphs (palette, lighting temperature, mood adjectives). Those help wide shots but hurt close-up identity. Keep close-ups tight on: subject identity descriptors → pose → minimal setting context → minimal lighting.
...
- Long character appearance paragraphs (full outfit breakdowns, multi-feature descriptions)

**ONE EXCEPTION — STILL REQUIRED in delta mode:** the **inline visual hook on each character's first mention** (the 4-8 word parenthetical from the INLINE VISUAL HOOK section above) — *because Klein loses identity binding across deltas without it*. Empirical: edit_previous_shot deltas WITHOUT the inline hook silently drift faces over successive shots. With the hook, identity holds.

✓ Correct delta: `"Angel (Black man, clean-shaven, short coiled hair) now lying supine on the asphalt, head tilted back, eyes looking up. A thin streak of blood traces from his temple."`
✗ Wrong delta: `"Angel now lying supine on the asphalt, head tilted back, eyes looking up."` ← no inline hook, Klein may drift identity

**LENGTH: 2-4 sentences MAXIMUM** (one sentence may be needed for the inline hooks themselves). Longer = you are re-describing the base image = FAIL.
